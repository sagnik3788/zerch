...
impl LocalEmbedder {
    ...
    pub fn embed(&mut self, text: &str) -> Result<Vec<f32>> {
        ...
        let view = outputs["last_hidden_state"].try_extract_array::<f32>()?; // Shape: [1, seq_len, 384]

        // Mean Pooling (Average across the sequence dimension)
        let seq_len = view.shape()[1];
        let mut sum = vec![0.0f32; 384];
        ...
        // Normalization (L2 Norm) - Critical for Cosine Similarity search
